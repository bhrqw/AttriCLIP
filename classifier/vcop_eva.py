...
class CoOp:
    ...
    @torch.no_grad()
    def accuracy(self, loader_1, num_test, test_class, mean_per_class=False):
        if mean_per_class:
            return self._accuracy_mpc(loader_1, num_test, test_class)
        else:
            return self._accuracy(loader_1, num_test, test_class)

    def _accuracy_mpc(self, loader_1, num_test, test_class):
        n_class = self.n_class
    ...
    def _accuracy(self, loader_1, num_test, test_class):
        total_count=0
        acc_count =0
        # pdb.set_trace()
        for i,(x, y) in enumerate(loader_1):
